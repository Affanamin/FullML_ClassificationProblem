...
#from xgboost.xgbclassifier import XGBRegressor
...
class Model_Finder:
    ...
    def get_best_params_for_random_forest(self,train_x,train_y):
        ...
        try:
            ...
            self.grid = GridSearchCV(estimator=self.clf, param_grid=self.param_grid, cv=3,n_jobs = -1,  verbose=2)
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            self.RandomForest= self.get_best_params_for_random_forest(train_x, train_y)
            self.prediction_RandomForest = self.RandomForest.predict(test_x) # Predictions using the decisionTreeReg Model
            self.accuracy_random_forest = round(self.RandomForest.score(train_x, train_y)*100,2)
            
            self.logger_object.log(self.file_object,'Got the model.')
            #random_forest.score(X_Train, Y_Train)
            #accuracy_random_forest = round(random_forest.score(X_Train, Y_Train) * 100, 2)
                        
            
            
            return 'RandomForestClassifier',self.RandomForest,self.prediction_RandomForest,self.accuracy_random_forest
